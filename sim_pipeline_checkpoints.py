...
class SimilarityPipeline:
    def __init__(self, reports_path, phrases_path, report_identifier='hash', phrase_field='sentence', label_field='label', threshold=0.75, batch_size=500, checkpoint_file='checkpoint.json'):
        ...
        self.processed_sentences_count = 0  # Track the number of processed sentences
    ...
    def vectorize_and_calculate_similarity(self, sentences):
        ...
        for batch_idx in range(num_batches):
            ...
            self.processed_sentences_count += (end_idx - start_idx)
    ...
    def save_checkpoint(self):
        """Saves the number of processed sentences to a checkpoint file."""
        with open(self.checkpoint_file, 'w') as file:
            json.dump({'processed_sentences_count': self.processed_sentences_count}, file)

    def load_checkpoint(self):
        """Loads the number of processed sentences from a checkpoint file."""
        if os.path.exists(self.checkpoint_file):
            with open(self.checkpoint_file, 'r') as file:
                checkpoint_data = json.load(file)
                self.processed_sentences_count = checkpoint_data.get('processed_sentences_count', 0)

    def run(self, full_text_output, adjacent_sentences_output=None, N=0):
        self.load_checkpoint()  # Load the checkpoint at the beginning
        self.load_data()
        self.preprocess_reports()
        sentences, hashes = self.flatten_sentences()

        # Only process the remaining sentences based on the checkpoint
        if self.processed_sentences_count < len(sentences):
            cosine_similarities = self.vectorize_and_calculate_similarity(sentences[self.processed_sentences_count:])
            ...
            print("All sentences have already been processed.")
